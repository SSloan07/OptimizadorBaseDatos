...
int main(void) {
    ...
    IntRecordHashTable *record_table_consulta = NULL;
    ...
    construir_ruta_bloque(location->id_comuna, location->id_bloque, ruta_consulta, sizeof(ruta_consulta));

    contenido_comprimido = leer_archivo_binario(ruta_consulta, &compressed_size);
    if (contenido_comprimido == NULL) {
        printf("No se pudo leer el archivo de consulta.\n");
        liberar_comunas();
        free_global_record_index(global_index);
        return 1;
    }
    ...
    if (contenido_descomprimido == NULL) {
        printf("No se pudo descomprimir el archivo de consulta.\n");
    ...
    record_table_consulta = cargar_bloque_en_hash_table(
        (const char *)contenido_descomprimido,
        RECORD_TABLE_SIZE
    );

    if (record_table_consulta == NULL) {
        printf("No se pudo cargar el bloque de consulta en la hash table.\n");
    ...
    record = search_int_record(record_table_consulta, id_registro);
    ...
    if (bloque_serializado == NULL) {
        printf("\nNo se pudo serializar la hash table destino.\n");
        free_int_record_hash_table(record_table_destino);
        free_int_record_hash_table(record_table_consulta);
        free(contenido_descomprimido);
        free(contenido_comprimido);
